...
Aplicativo desktop de produção editorial com pesquisa, artigo, carrossel e aprovação humana. **0.1.0-alpha.10 — esboço experimental.** Beta só começa com autorização explícita de Rodrigo para uso cotidiano dos desenvolvedores.
...
5. Para Instagram, desbloqueie o cofre e clique em **Conectar Instagram**. A distribuição precisa ter o serviço OAuth habilitado pelo mantenedor; veja [implantação](auth-service/README.md). O usuário autoriza perfil e publicação no Instagram, sem copiar IDs ou tokens. O serviço está implantado no Cloudflare e o OAuth Instagram foi validado com uma conta profissional. O aplicativo renova a autorização quando possível e hospeda os JPEGs temporariamente para a publicação pela Meta.
