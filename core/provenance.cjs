    ...
    .update(
      JSON.stringify({
        ...
        article: reviewedChannels.includes("blog")
          ? revision?.article
          : undefined,
        caption: reviewedChannels.includes("instagram")
          ? revision?.caption
          : undefined,
        cards: reviewedChannels.includes("instagram")
          ? revision?.cards
          : undefined,
...
function reviewFeedback(project, revision = project.revisions.at(-1)) {
  const reviews = project.messages.filter(
    (m) => m.agent === "reviewer" && m.role === "assistant" && !m.internal,
  );
  const exact = [...reviews]
    .reverse()
    .find((m) => m.revisionId === revision?.id && m.reviewFingerprint);
  ...
  const valid =
    !!revision &&
    !!found.revisionId &&
    found.revisionId === revision.id &&
    !!reviewedChannels.length &&
    found.reviewFingerprint ===
      reviewFingerprint(project, revision, reviewedChannels);
  return {
    status: valid ? "current" : found.revisionId ? "stale" : "legacy",
    content: found.content,
    at: found.at,
    revisionId: found.revisionId,
    channels: reviewedChannels,
  };
}
