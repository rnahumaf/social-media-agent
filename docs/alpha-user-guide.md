...
## Condições deste alfa

O responsável precisa cadastrar seu @ como testador na Meta e sua conta Google na lista de testes antes do primeiro acesso. Esse cadastro já foi preparado para o primeiro testador convidado. O Instagram exige que você aceite o convite na sua própria conta. Contas Instagram pessoais comuns não são compatíveis com a API de publicação.

O Google pode mostrar o domínio **rnahumaf.workers.dev** na autorização: é o serviço de conexão deste alfa. A configuração Google está em testes e pode exigir nova autorização após sete dias. Se o acesso for negado, confira a conta escolhida e informe a mensagem ao responsável, sem enviar senhas ou códigos. Se o blog não aparecer, confirme que a conta escolhida tem acesso de autor ou administrador no Blogger.

Os executáveis ainda não têm assinatura de código; o pacote macOS também não é notarizado. O alfa é experimental. As conexões Instagram e WordPress.com foram consultadas em contas reais; o Blogger foi verificado com testes automatizados e configuração do serviço. A primeira autorização real da conta do testador será feita por ele. Nenhuma publicação real foi usada na validação.
...
Ao convidar outra pessoa, entregue somente o aplicativo e este guia. Cada testador deve criar seu próprio workspace; não distribua sua pasta de trabalho nem seu cofre.
