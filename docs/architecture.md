...
`CardTextEditor` usa um subconjunto de documentos Tiptap para título e corpo dos cards, validado por `core/card-text-schema.cjs`. `titleRich` e `bodyRich` são opcionais e acompanham o texto simples idêntico; revisões antigas continuam legíveis sem migração. A renderização preserva estilos, listas, quebras e recuos em SVG/JPEG. Geração e reescrita recebem o contrato de formatação, e alterações somente visuais participam da comparação de propostas e da aprovação. O contrato e a cobertura ficam em [texto rico e progresso](rich-cards-and-progress.md).

`core/run-progress.mjs` deriva a timeline do cursor, status, modo e canais da sessão persistida. A interface não estima a duração nem avança etapas por temporizador. Tentativas repetidas mantêm as etapas anteriores concluídas; pausa, interrupção e cancelamento suspendem o indicador animado.
