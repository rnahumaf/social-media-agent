...
No título e no texto de cada card, selecione um trecho e use **Negrito**, **Itálico** ou **Sublinhado**. Os botões **Lista com marcadores** e **Lista numerada** organizam os parágrafos. **Aumentar recuo** e **Diminuir recuo** ajustam parágrafos ou criam níveis dentro das listas; o primeiro item precisa de um item anterior para ser aninhado. **Desfazer** e **Refazer** recuperam alterações. A prévia mostra o JPEG que será exportado. Os limites de 90 e 420 caracteres contam o texto visível e as quebras de linha; se o conteúdo não couber com legibilidade, distribua-o entre mais cards. O agente também recebe essas opções ao gerar ou reescrever.
...
A timeline acima dos editores mostra a etapa em execução, as concluídas e as próximas, sem precisar abrir os detalhes. A sequência inclui apenas os canais da geração; adaptar um artigo salvo dispensa as etapas de pesquisa e evidências. O indicador acompanha o estado salvo: ele para em caso de pausa ou cancelamento e continua da etapa correspondente quando você retoma. A animação respeita a preferência do sistema por movimento reduzido.

Os cards são renderizados em JPEG 1080 × 1350. Em **Estilo do carrossel**, o controle **Tamanho da fonte** ajusta título e corpo entre 85% e 115% e pode ser salvo como padrão do autor. O template ainda escolhe automaticamente uma base que caiba no card, preservando margens e rodapé. Se o tamanho escolhido não comportar o texto, divida o conteúdo em mais cards.
