...
global.fetch = async (url, options = {}) => {
  ...
  if (u.hostname === "example.test") {
    if (options.method && options.method !== "GET") {
      postCalls++;
      throw Error("Unexpected publication from a recovery check");
    }
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const manual = seed.create("Conferência manual P0", "Teste", "", {
      channels: ["blog"],
      manual: true,
    });
    seed.revise(manual.id, {
      article: "# Conteúdo para conferir",
      caption: "",
      cards: [],
    });
    manual.publications.wordpress = {
      status: "uncertain",
      revision: current(manual).id,
      title: manual.title,
      destination: "https://example.test",
    };
    const p = seed.create("Pauta P0", "Teste editorial", "", {
      channels: ["blog", "instagram"],
      manual: true,
    });
    const oldSource = {
      title: "Fonte da revisão",
      pmid: "1",
      abstract: "Original",
      access: "abstract",
    ...
    const newSource = {
      title: "Pesquisa posterior",
      pmid: "2",
      abstract: "Nova",
      access: "abstract",
      url: "https://example.test/new",
    };
    const first = seed.revise(p.id, {
    ...
    p.messages.push({
      role: "assistant",
      agent: "reviewer",
      content: "Avaliação antiga.",
      revisionId: first.id,
      reviewedChannels: ["blog", "instagram"],
      reviewFingerprint: reviewFingerprint(p, first, ["blog", "instagram"]),
    ...
    p.sessions = [
      {
        ...
        artifacts: {
          sources: [newSource],
          searches: [{ provider: "pubmed", query: "new evidence", count: 1 }],
        },
      },
    ];
    Object.assign(seed.state.settings, {
      ...
      instagramAccount: "456",
      instagramUsername: "fixture",
    });
    ...
    seed.save();
    seed.close();
    attemptedHTML = (await import("../core/blog-html.mjs")).renderBlog(
      first.article,
    );
    ...
    await call("vault", {
      ...
      values: { openrouter: "fixture", wordpress: "fixture" },
      remember: false,
    });
    await reload();
    ...
    assert.equal(
      await evaluate(
        "document.querySelector('.review-feedback').dataset.reviewStatus",
      ),
      "stale",
    );
    assert.equal(
      await evaluate(
        "[...document.querySelectorAll('[data-destination=instagram] button')].some(b=>b.textContent.includes('Publicar'))",
      ),
      false,
    );
    assert.ok(
      await evaluate(
        "document.querySelector('[data-destination=instagram]').textContent.includes('anterior')",
      ),
    );
    ...
    await wait(
      async () => (await call("state")).operation?.name === "reconcile",
      "recovery operation",
    );
    ...
    assert.equal(
      await evaluate(
        "!!document.querySelector('[data-testid=cancel-operation]')",
      ),
      false,
    );
    await assert.rejects(
      call("cancel", { operationId: recovery.id }),
      /não pode/,
    );
    ...
    assert.equal(
      await evaluate(
        "document.querySelector('.content-pad > .source h3').textContent",
      ),
      "Fonte da revisão",
    );
    assert.equal(
      await evaluate(
        "document.querySelector('.pending-research .source h3').textContent",
      ),
      "Pesquisa posterior",
    );
    ...
    await dom(
      "!!document.querySelector('[data-testid=cancel-operation]:not(:disabled)')",
    );
    ...
    assert.equal(
      state.projects[0].messages.filter(
        (m) => m.content === "Pedido cancelável",
      ).length,
      1,
    );
    assert.equal(
      state.projects[0].messages.find((m) => m.content === "Pedido cancelável")
        .status,
      "cancelled",
    );
    ...
    await assert.rejects(
      call("cancel", { operationId: chat.id }),
      /já terminou/,
    );
    ...
    await evaluate(
      "[...document.querySelectorAll('.projects button')].find(b=>b.textContent.includes('Conferência manual P0')).click()",
    );
    await dom(
    ...
    assert.equal(
      await evaluate(
        "document.querySelector('.manual-recovery button').disabled",
      ),
      true,
    );
    await evaluate(
      "document.querySelector('.manual-recovery summary').click(); document.querySelector('.manual-recovery input').click()",
    );
    await click("Registrar conferência manual");
    await dom(
      "document.querySelector('[data-destination=wordpress]').textContent.includes('conferência manual, não uma confirmação da API')",
    );
    const checked = (await call("state")).projects.find(
    ...
    assert.equal(postCalls, 0);
    console.log(
      "P0 desktop passed: recovery without publishing, update state, immutable Instagram, revision-bound sources/reviews, noncancellable actions, cancellation identity, chat retry, editable rewrite cancellation, 780px layout.",
    );
    win.destroy();
    app.quit();
  } catch (error) {
