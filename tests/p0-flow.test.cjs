...
async function fixture(t) {
  ...
  const p = w.create("Tema", "Público geral", "", {
...
test("P0 WordPress reconciliation binds the attempted revision, not a newer edit", async (t) => {
  ...
  mockRequest(t, async (url, options) => {
    ...
    assert.match(url, /\/posts\/77\?context=edit$/);
    return {
      id: 77,
      status: "publish",
      title: { raw: p.title },
      content: { raw: renderBlog(previousRevision.article) },
...
test("P0 Blogger rejects a post from a different blog", async (t) => {
  ...
  await assert.rejects(
    reconcile(w, { id: p.id, channel: "blogger", remoteId: "77" }),
    /não confirmou/,
  );
...
test("P0 Instagram checks never republish: FINISHED is not PUBLISHED", async (t) => {
  ...
  mockRequest(t, async (url, options) => {
    calls++;
    assert.equal(options.method, undefined);
    assert.match(url, /789\?fields=status_code$/);
...
test("P0 Instagram interrupted before submission becomes explicitly retryable", async (t) => {
  ...
  mockRequest(t, async () => {
    throw Error("No remote call needed before submission");
  });
  await reconcile(w, { id: p.id, channel: "instagram" });
...
test("P0 Instagram manual binding checks account ownership and does not claim equal images", async (t) => {
  ...
  await assert.rejects(
    reconcile(w, { id: p.id, channel: "instagram", remoteId: "99" }),
    /não foi encontrado/,
  );
...
test("P0 failed regeneration preserves previous revision sources and review validity", async (t) => {
  ...
  providers.complete = async () => {
    ...
    if (calls === 1)
      return {
        content: '{"provider":"pubmed","query":"new search"}',
        model: "fixture",
        usage: {},
      };
...
test("P0 generation persists exact review provenance and search snapshot", async (t) => {
  ...
  const outputs = [
    '{"provider":"pubmed","query":"evidence"}',
    "Dossiê",
    "# Artigo novo\n\nConteúdo.",
    "Revisão vinculada.",
  ];
  ...
  assert.equal(
    p.messages.find((m) => m.agent === "reviewer" && m.role === "assistant")
      .revisionId,
    reviewed,
  );
});
...
test("P0 manual absence confirmation preserves the attempt and never calls publishing APIs", async (t) => {
  ...
  mockRequest(t, async () => {
    throw Error("Manual confirmation must not call a remote API");
  });
  await assert.rejects(
    reconcile(w, { id: p.id, resolution: "not-published" }),
    /Confirme/,
  );
  ...
  assert.equal(
    p.publications.wordpress.manualCheck.resolution,
    "not-published",
  );
});
test("P0 connection verification forwards cancellation to every provider", async (t) => {
  ...
  const pending = [
    require("../core/instagram-auth.cjs").profile(
      "fixture",
      "v23.0",
      controller.signal,
    ),
    require("../core/wordpress-auth.cjs").profile(
      "fixture",
      "123",
      "https://example.test",
      controller.signal,
  ...
  for (const result of results) {
    ...
    assert.match(result.reason.message, /Cancelled fixture/);
  }
});
