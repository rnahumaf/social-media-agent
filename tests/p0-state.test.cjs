...
test("P0 published Instagram cannot be submitted again, including a newer revision", async () => {
  ...
  project.publications.instagram = {
    status: "published",
    revision: "r1",
    destination: "456",
  };
  ...
  assert.match(view.status, /anterior/);
  assert.equal(view.canPublish, false);
});
test("P0 blog publication distinguishes published, update, failed and unknown revisions", async () => {
  ...
  for (const channel of ["wordpress", "blogger"]) {
    ...
    assert.equal(view.mode, "update");
    assert.match(view.status, /não publicadas/);
    project.publications[channel].status = "uncertain";
    view = publicationView(project, state, channel, {
      dirty: true,
      desktop: true,
    });
    assert.equal(view.mode, "recover");
...
test("P0 publication respects title changes, connection, destination, dirty and invalid cards", async () => {
  const { publicationView } = await import("../core/publication-view.mjs");
  const { project, state } = fixture();
  project.publications.wordpress = {
    status: "published",
    revision: "r1",
    remoteId: "9",
  ...
  assert.equal(
    publicationView(project, state, "instagram", { desktop: true }).mode,
    "connect",
  );
});
test("P0 review belongs to exact content, title, briefing and channels", () => {
  ...
  project.messages.push({
    ...
    reviewedChannels: ["blog"],
    reviewFingerprint: reviewFingerprint(project, revision, ["blog"]),
  });
  assert.equal(reviewFeedback(project).status, "current");
  project.title = "Outro tema";
  assert.equal(reviewFeedback(project).status, "stale");
  project.title = "Tema";
  project.brief = "Outra demanda";
  assert.equal(reviewFeedback(project).status, "stale");
  project.brief = "Público geral";
  project.revisions.push({ ...revision, id: "r2" });
  assert.equal(reviewFeedback(project).status, "stale");
...
test("P0 only definite rejections or pre-submission failures become retryable", () => {
  ...
  assert.equal(
    failureStatus(Error("container creation failed"), false),
    "failed",
  );
});
...
test("P0 missing publication authorization exposes configuration instead of a rejected action", async () => {
  ...
  for (const channel of ["wordpress", "blogger", "instagram"]) {
    ...
    assert.equal(
      publicationView(project, state, channel, { desktop: true }).mode,
      "connect",
    );
