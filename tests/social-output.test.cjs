...
test("local fallback changes only lengths and refuses malformed structures", () => {
  const source = JSON.stringify(
    valid({
      cards: [
        { title: "Título ".repeat(20), body: "texto ".repeat(100) },
  ...
  assert.ok(fitted.cards[0].title.length <= 90);
  assert.ok(fitted.cards[0].body.length <= 420);
  assert.ok(fitted.cards[0].title.endsWith("…"));
